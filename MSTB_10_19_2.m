...
for indexImg = 204:204
    ...
    for i=begin_index:1:20
        ...
        %先将完全被包含的抑制掉
        thresh=0.9;
        [selectedBbox,~,selectIndex] = selectStrongestBbox(bbox,bboxArea,'RatioType','Min','OverlapThreshold',thresh);        
        ...
        %2016-10-19-2：策略2： 先分类器，再文本行聚类
       edgeStats=edgeStats(selectIndex);
        for ii=1:length(edgeStats)
            
            gBbox=edgeStats(ii).Image;
            fgb=figure(ii)
            subplot(1,3,1);
            imshow(gBbox);
