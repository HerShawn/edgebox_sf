...
for c = 1 : 3                 %3个通道，r，g，b分别画
    for dl = 1 : lineSize   %线的宽度，线条是向外面扩展的
        d = dl - 1;
        if  1==flag %有缺口的框
            dest(  max(y1-d,3) ,            max(x1,3):min((x1+wx),xA-3) ,  c  ) =  color(c); %上方线条
            dest(  min(y1+wy+d,yA-3) ,     max(x1,3):min((x1+wx),xA-3), c  ) =  color(c); %下方线条
            dest(  max(y1,3):min((y1+wy),yA-3) ,   max(x1-d,3) ,           c  ) =  color(c); %左方线条
            dest(  max(y1,3):min((y1+wy),yA-3)  ,   min(x1+wx+d,xA-3) ,    c  ) =  color(c); %左方线条
        elseif 2==flag %无缺口的框
